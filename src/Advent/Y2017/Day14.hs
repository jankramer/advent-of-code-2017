...
day14a input = show $ length . filter (== '1') $ concat $ grid input
day14b input = show $ length . components $ g
    where (g, _, _) = graphFromEdges $ graphEntries $ grid input

grid :: String -> [String]
grid input = map (knotHashToBinary . knotHash) (map (\x -> input ++ ('-' : show x)) [0..127])
...
neighbours (x,y) = filter (\(x,y) -> x >= 0 && x <= gridSize && y >= 0 && y <= gridSize) [(x, y-1), (x-1, y), (x+1, y), (x, y+1)]
